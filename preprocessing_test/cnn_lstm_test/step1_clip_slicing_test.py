...
logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s - %(levelname)s - [STEP1_CLIP_SLICING_TEST] %(message)s',
    handlers=[
...
# Hàm xử lý toàn bộ video trong thư mục test
def process_test_videos(root_input_dir, root_output_dir, verbose=True):
    start_time = time.time()
    total_clips = 0
    for label in os.listdir(root_input_dir):
        ...
        for video_folder in os.listdir(label_path):
            ...
            if verbose:
                logging.info(f"Đang xử lý test: {input_dir}")
            clip_count = create_clips_from_frames(input_dir, output_dir, verbose)
            total_clips += clip_count
    if verbose:
        logging.info(f"Hoàn tất tạo {total_clips} clip (test) trong {time.time() - start_time:.2f} giây")

if __name__ == "__main__":
    process_test_videos(
        "processed_data/frames_test", 
        "processed_data/results_cnn_lstm_test/cnn_lstm_clips_test", 
        verbose=True
    )
